...
import { Toaster } from "@/components/ui/toaster"; // 或 - import LoginPage from "@/pages/Login";
//   ✅ 引入New页面
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        {/* 注意：Must放在 "/*" 之前，否则会被 MainLayout 拦截 */}
        <Route 
          path="/automation/edit/:id" 
          element={
            <RequireAuth>
              {/* 这里不包 MainLayout，直接渲染全屏Edit器 */}
              <EditPlaybookPage />
            </RequireAuth>
          } 
        />

        {/* 4. 主Application路由 (受保护，带侧边栏Sum Tab System) */}
        <Route
          path="/*"
          element={
            <RequireAuth>
