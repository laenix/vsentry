...
export function LogGroupView({ data }: { data: VLResult[] }) {
  ...
  useEffect(() => {
    ...
    if (isRawLog) {
      // Log模式: Default显示Time、消息
      // 检查当ago visibleColumns 是否有效，如果Invalid则重置
  ...
  const groups = useMemo(() => {
    ...
    data.forEach(log => {
      // 聚合结果通常没有 _stream 字段，我们将其归Class为 "{}"
      const s = log._stream || "{}" 
      if (!map[s]) map[s] = []
      map[s].push(log)
    })
  ...
  // 自动Select第一个分Group
  useEffect(() => {
    // 只有当没有选Medium，或者选Medium的流在当agoDataMediumNot found时，才自动Select第一个
        ...
        <div ref={detailParentRef} className="flex-1 overflow-auto scrollbar-thin scrollbar-thumb-muted-foreground/20">
           ...
           >
             {rowVirtualizer.getVirtualItems().map((virtualRow) => (
               ...
               >
                 {/* 确保 LogEntryItem 能Receive visibleColumns 并动态Render */}
                 <LogEntryItem 
