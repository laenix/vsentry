...
// ListIngest - func ListIngest(ctx *gin.Context) {
	db := database.GetDB()
...
func AddIngest(ctx *gin.Context) {
	...
	ctx.JSON(200, gin.H{"code": 200, "msg": "AddSuccess"})
}

// UpdateIngest - func UpdateIngest(ctx *gin.Context) {
	...
	if err := db.Model(&model.Ingest{}).Where("id = ?", req.ID).Updates(req).Error; err != nil {
		ctx.JSON(500, gin.H{"msg": "Update failed"})
		return
	}

	//   3. 【关键】清理 Badger Medium的 Token Cache
	//   这样下次Log进来时，Medium间件会重New从 SQLite Load最New的 StreamFields
	for _, auth := range auths {
		database.DelTokenCache(auth.SecretKey)
	}

	ctx.JSON(200, gin.H{"code": 200, "msg": "UpdateSuccess，Cache已Sync"})
}

// DeleteIngest - func DeleteIngest(ctx *gin.Context) {
	id := ctx.Query("id")
	db := database.GetDB()
	...
	// 清理Cache - _, auth := range auths {
		database.DelTokenCache(auth.SecretKey)
	}

	db.Delete(&model.Ingest{}, id)
	ctx.JSON(200, gin.H{"code": 200, "msg": "DeleteSuccess"})
}

// GetIngestAuth - Ingest 的 Token
