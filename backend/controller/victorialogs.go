...
// QueryVictoriaLogs - VictoriaLogs
func QueryVictoriaLogs(ctx *gin.Context) {
	...
	// VictoriaLogs - ISO 格式Time（不带 Z 后缀或毫秒）或 Unix Time戳
	if start != "" {
		// 移除 - 时区的 Z 后缀Sum毫秒
	...
	// CreateRequest - , err := http.NewRequest("POST", targetURL, ctx.Request.Body)
	if err != nil {
	...
	// 透传Response - .Header("Content-Type", resp.Header.Get("Content-Type"))
	io.Copy(ctx.Writer, resp.Body)
}

// QueryVictoriaLogsHits - func QueryVictoriaLogsHits(ctx *gin.Context) {
	vlURL := viper.GetString("victorialogs.url")
	if vlURL == "" {
		vlURL = "http://  localhost:9428"
	}
...
// GetVictoriaLogsMetrics - VictoriaLogs 指标
...
func GetVictoriaLogsHealth(ctx *gin.Context) {
	...
	// 尝试访问Health - resp, err := http.Get(vlURL + "/health")
...
// ProxyVictoriaLogsSelect - （透传所有 /select/* Request）
func ProxyVictoriaLogsSelect(ctx *gin.Context) {
	...
	// Transfer原始Request - := vlURL + "/select/" + path
	
	//   如果有QueryParameter，追加上去
	...
	// 复制Request头 - k, v := range ctx.Request.Header {
		for _, vv := range v {
			req.Header.Add(k, vv)
		}
	}
	...
	// 透传Response头 - k, v := range resp.Header {
		for _, vv := range v {
			ctx.Header(k, vv)
		}
	}
