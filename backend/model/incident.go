...
type Incident struct {
	...
	//   核心：一对多关联。前端在拿到 Incident Detail时，Can直接拿到这个数Group
...
type Alert struct {
	gorm.Model
	IncidentID  uint   `json:"incident_id"` // 外键 - uint   `json:"rule_id"`
	Content     string `json:"content"` // Storage - 搜出的原始 JSON Data
