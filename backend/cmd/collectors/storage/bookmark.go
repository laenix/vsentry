...
// NewBookmarkManager Initialize书签Manage器并加载本地记录
...
func (b *BookmarkManager) Save() {
	...
	// 防止写入一半时Service器掉电或 Agent 被 Kill 导致 JSON File损坏
	tempFile := b.filePath + ".tmp"
	os.WriteFile(tempFile, data, 0644)
	os.Rename(tempFile, b.filePath)
}

// UpdateOffset UpdateFile游标 (针对 Linux/macOS)
...
// UpdateRecordID UpdateEvent记录 ID (针对 Windows)
