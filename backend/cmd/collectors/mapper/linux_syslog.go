...
var (
	// 匹配标准 Syslog 格式，提取：时间、主机名、进程名、PID、日志内容
...
func mapSyslog(line string, entry *ocsf.VSentryOCSFEvent) {
	...
	if len(matches) >= 6 {
		...
		// 简单的异常定性引擎：如果内核/服务日志里包含 error 关键字，直接拉高告警级别
...
// guessTime 智能时间推算函数（解决传统 syslog 不带年份的痛点）
func guessTime(timeStr string) string {
	...
	// 如果解析出的时间在未来（比如现在是1月，日志是12月，那说明日志是去年的）
	if parsed.After(now) {
