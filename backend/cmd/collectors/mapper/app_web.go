...
var (
	...
	// Nginx 错误日志格式: 2026/03/03 10:00:00 [error] 12345#0: *6789 open() failed...
	nginxErrorRe = regexp.MustCompile(`^\d{4}/\d{2}/\d{2}\s+\d{2}:\d{2}:\d{2}\s+\[(\w+)\]\s+(.*)$`)

	// Apache 错误日志格式: [Tue Mar 03 10:00:00.123 2026] [core:error] [pid 1234] [client 1.2.3.4:5678] message
...
func mapWebAccess(line string, entry *ocsf.VSentryOCSFEvent) {
	...
	if len(matches) >= 12 {
		...
		// 1. 拦截明显的 Web 攻击特征 (SQLi, XSS, 目录穿越, 敏感文件读取)
