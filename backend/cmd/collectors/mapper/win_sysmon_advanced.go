...
func mapSysmonDNS(unmapped map[string]interface{}, entry *ocsf.VSentryOCSFEvent) {
	...
	entry.ClassUID = 4003 // OCSF - Activity
...
func mapSysmonRegistry(unmapped map[string]interface{}, entry *ocsf.VSentryOCSFEvent) {
	...
	entry.ClassUID = 1010 // OCSF - eventID := entry.Unmapped["event_id"].(int)
...
func mapSysmonInjection(unmapped map[string]interface{}, entry *ocsf.VSentryOCSFEvent) {
	...
	// 发起攻击的Process - .Actor = &ocsf.User{Name: GetStr(unmapped, "SourceImage")}
	...
	if eventID == 8 {
		...
		entry.Unmapped["granted_access"] = GetStr(unmapped, "GrantedAccess") // 如 - (全部Permission)
	}
}
